...
# 唐乃亥影响前期天然径流
tnh_preinfl = pd.read_csv('data/tangnaihai_monthly_flow.csv', parse_dates=['date'],index_col='date')
...
# 唐乃亥影响期实测径流
tnh_infl_obs = pd.read_csv('data/tangnaihai_monthly_flow.csv', parse_dates=['date'],index_col='date')
...
# 贵德影响期天然径流
guide_infl = pd.read_csv('data/guide_monthly_naturalized_flow_viflstm.csv', parse_dates=['date'],index_col='date')
guide_infl['date_'] = guide_infl.index
guide_infl['days'] = guide_infl['date_'].apply(Get_days_from_month)
guide_infl['runoff(10^8m^3)'] = guide_infl['VIF_LSTM']*guide_infl['days']*24*3600/100000000
...
# 循化影响期天然径流
xunhua_infl = pd.read_csv('data/xunhua_monthly_naturalized_flow_viflstm.csv', parse_dates=['date'],index_col='date')
xunhua_infl['date_'] = xunhua_infl.index
xunhua_infl['days'] = xunhua_infl['date_'].apply(Get_days_from_month)
xunhua_infl['runoff(10^8m^3)'] = xunhua_infl['VIF_LSTM']*xunhua_infl['days']*24*3600/100000000
...
# 创建一个绘图区域，包含两个子图
fig, axes = plt.subplots(nrows=3, ncols=2, figsize=(7.48, 7.48))
...
axes[0][0].set_title('(a) Period 1956-1985 at Tangnaihai', fontsize=ftsize)
...
axes[0][1].set_title('(b) Period 1986-2018 at Tangnaihai', fontsize=ftsize)
...
axes[1][0].set_title('(c) Pre-influenced Period at Guide', fontsize=ftsize)
...
# 子图2：影响期
axes[1][1].bar(guide_infl.index - 0.2, guide_infl['runoff(10^8m^3)'], width=0.4, color=naturalized_runoff_color, label='Naturalized runoff')
...
axes[1][1].set_title('(d) Influenced Period at Guide', fontsize=ftsize)

# 创建第二个y轴
ax2_4 = axes[1][1].twinx()
ax2_4.plot(guide_infl.index, guide_infl['Ratio'], color=rario_naturalized_runoff_color, label='Ratio of annual naturalized runoff')
...
axes[2][0].set_xlabel('Month', fontsize=ftsize)
axes[2][0].set_ylabel('Runoff($10^8m^3$)', fontsize=ftsize)
axes[2][0].set_title('(e) Pre-influenced Period at Xunhua', fontsize=ftsize)

# 创建第二个y轴
ax2_5 = axes[2][0].twinx()
ax2_5.plot(xunhua_preinfl.index, xunhua_preinfl['Ratio'], color=ratio_natural_runoff_color)
ax2_5.set_ylabel('Ratio (%)', fontsize=ftsize)

# 子图4：循化影响后期径流
axes[2][1].bar(xunhua_infl.index - 0.2, xunhua_infl['runoff(10^8m^3)'], width=0.4, color=naturalized_runoff_color)
axes[2][1].bar(xunhua_infl.index + 0.2, xunhua_infl_obs['runoff(10^8m^3)'], width=0.4, color=observed_runoff_color)
axes[2][1].set_xlabel('Month', fontsize=ftsize)
axes[2][1].set_ylabel('Runoff($10^8m^3$)', fontsize=ftsize)
axes[2][1].set_title('(f) Influenced Period at Xunhua', fontsize=ftsize)

# 创建第二个y轴
ax2_6 = axes[2][1].twinx()
ax2_6.plot(xunhua_infl.index, xunhua_infl['Ratio'], color=rario_naturalized_runoff_color)
ax2_6.plot(xunhua_infl_obs.index, xunhua_infl_obs['Ratio'], color=ratio_observed_runoff_color)
ax2_6.set_ylabel('Ratio (%)', fontsize=ftsize)
