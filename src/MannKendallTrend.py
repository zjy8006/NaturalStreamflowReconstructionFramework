...
# added by Jianyi Zuo
def plot_trend(x,**kwargs) -> plt.Axes:
    ...
    * `ax`: [`Axes`]:
    ...
    series_color = kwargs.get('series_color',None)
    trend_color = kwargs.get('trend_color',None)
    ...
    trend, h, p, z, Tau, s, var_s, slope, intercept = original_test(x)
    equation = '$y={}x+{}$'.format(round(slope,2),round(intercept,2))
    try:
    ...
    x_vals = [i for i in range(len(idx))]

    trend_line = [slope*x_vals[i] + intercept for i in range(len(x_vals))]
    ax.plot(idx,x,c=series_color,label=series_name)
    # ax.plot(idx,trend_line,'--',c=trend_color,label='Trend of {}\n{}\np={};z={}; $Z_(0.05)$=\\plusmn 1.96'.format(series_name,equation,round(p,3),round(z,3)))
    ax.plot(idx,trend_line,'--',c=trend_color,label=r'Trend of '+series_name+'\n'+equation+'\np='+str(round(p,3))+'\nz='+str(round(z,3))+'\n$Z_{0.05}=\pm1.96$')
    ...
    ax.set_ylabel('Records (${}$)'.format(unit))
